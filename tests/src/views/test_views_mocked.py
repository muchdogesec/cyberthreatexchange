...
class TestSearchViewList:
    ...
    def test_calls_arango_helper_semantic_search(self, client):
        ...
        with patch("cyberthreatexchange.server.views.ArangoDBHelper.semantic_search") as mock_semantic_search:
...
class TestFeedObjectsView:
    ...
    def test_sdos_filters_by_sdo_types(self, client, feed):
        ...
        with patch("cyberthreatexchange.server.views.ArangoDBHelper.semantic_search") as mock_semantic_search:
            ...
            # Verify
            mock_semantic_search.assert_called_once_with([feed.collection_name], valid_types=SDO_TYPES)
    ...
    def test_scos_filters_by_sco_types(self, client, feed):
        ...
        with patch("cyberthreatexchange.server.views.ArangoDBHelper.semantic_search") as mock_semantic_search:
            ...
            # Verify
            mock_semantic_search.assert_called_once_with([feed.collection_name], valid_types=SCO_TYPES)
    ...
    def test_smos_filters_by_smo_types(self, client, feed):
        ...
        with patch("cyberthreatexchange.server.views.ArangoDBHelper.semantic_search") as mock_semantic_search:
            ...
            # Verify
            mock_semantic_search.assert_called_once_with([feed.collection_name], valid_types=SMO_TYPES)
    ...
    def test_sros_filters_by_relationship_type(self, client, feed):
        ...
        with patch("cyberthreatexchange.server.views.ArangoDBHelper.semantic_search") as mock_semantic_search:
            ...
            # Verify
            mock_semantic_search.assert_called_once_with([feed.collection_name], valid_types=["relationship"])
    
    def test_retrieve_gets_object_by_external_id(self, client, feed):
        """Test that retrieve method gets object by external ID."""
        with patch("cyberthreatexchange.server.views.ArangoDBHelper.get_object_by_external_id") as mock_get_object:
            mock_get_object.return_value = Response(
                {'type': 'malware', 'id': 'malware--123'}, 
                status=200
            )
    ...
    def test_versions_gets_object_versions(self, client, feed):
        ...
        with patch("cyberthreatexchange.server.views.ArangoDBHelper.get_versions") as mock_get_versions:
            mock_get_versions.return_value = Response(
                {'latest': '2020-01-15T10:00:00.000Z', 'versions': []}, 
                status=200
            )
            
            # Execute
            response = client.get(f'/api/v1/feeds/{feed.id}/objects/malware--123/versions/')
    ...
    def test_bundle_gets_object_with_bundle_flag(self, client, feed):
        ...
        with patch("cyberthreatexchange.server.views.ArangoDBHelper.get_object_by_external_id") as mock_get_object:
            ...
            # Execute
            response = client.get(f'/api/v1/feeds/{feed.id}/objects/malware--123/bundle/')
    ...
    def test_list_raises_404_when_feed_not_found(self, client):
        ...
        # Execute
        response = client.get('/api/v1/feeds/00000000-0000-0000-0000-000000000000/objects/')
    ...
    def test_retrieve_raises_404_when_feed_not_found(self, client):
        ...
        # Execute
        response = client.get('/api/v1/feeds/00000000-0000-0000-0000-000000000000/objects/malware--123/')
